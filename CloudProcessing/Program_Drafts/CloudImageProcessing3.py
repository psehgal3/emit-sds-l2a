...
def read_image_file(infile):
    ...
    array_3d = np.array(array_3d)
    #array_3d = list(array_3d)
    array_3d = np.ndarray.squeeze(array_3d, axis=1)
    #array_3d = np.ufunc.reduce(array_3d)
...
if __name__ == "__main__":
    ...
    for test_infile in files[number_train_files:len(files)]:
        ...
        predictions = predict(model, new_array_3d)
        create_new_classification_map(predictions, new_array_3d, annotation_array)